...
const security_headers_1 = require("./utils/security-headers");
...
const systemRouter = new router_1.Router();
systemRouter.setupSwagger(swagger_1.swaggerSpec);
/**
...
const server = http_1.default.createServer(async (req, res) => {
    try {
        (0, security_headers_1.applySecurityHeaders)(res);
        if (req.url && req.url.startsWith("/api-docs")) {
            res.setHeader("Content-Security-Policy", "default-src 'self'; script-src 'self' 'unsafe-inline' https://unpkg.com; style-src 'self' 'unsafe-inline' https://unpkg.com; img-src 'self' data: https://validator.swagger.io;");
        }
        res.setHeader("Access-Control-Allow-Origin", env_1.env.FRONTEND_ORIGIN);
        ...
        if (docsHandled)
            return;
        const handled = await auth_routes_1.authRouter.handle(req, res);
        if (handled)
            return;
        (0, json_1.sendJson)(res, 404, { ok: false, message: "Ruta no encontrada" });
    }
    catch (error) {
...
async function bootstrap() {
    try {
        ...
        server.listen(env_1.env.PORT, () => {
            ...
            console.log(`📄 Swagger docs disponibles en: http://localhost:${env_1.env.PORT}/api-docs`); // 👈 Un pequeño log extra para encontrarlo rápido
        });
        void (0, mailer_1.warmupMailer)();
    }
    catch (error) {
