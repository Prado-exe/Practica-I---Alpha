...
exports.getRolesWithDetailsFromDb = getRolesWithDetailsFromDb;
exports.getAllPermissionsFromDb = getAllPermissionsFromDb;
exports.createRoleWithPermissionsInDb = createRoleWithPermissionsInDb;
exports.updateRoleWithPermissionsInDb = updateRoleWithPermissionsInDb;
exports.deleteRoleAndReassignUsersInDb = deleteRoleAndReassignUsersInDb;
/**
 * ============================================================================
 ...
 */
async function getRolesWithDetailsFromDb() {
    const query = `
    SELECT 
 ...
 * @throws {Ninguna}
 */
async function getAllPermissionsFromDb() {
 ...
 */
async function createRoleWithPermissionsInDb(code, name, description, permisos) {
    const client = await db_1.pool.connect();
    try {
        await client.query('BEGIN');
        // Insertamos el rol
        const roleRes = await client.query(`INSERT INTO roles (code, name, description) VALUES ($1, $2, $3) RETURNING role_id`, [code, name, description]);
 ...
 */
async function updateRoleWithPermissionsInDb(roleId, code, name, description, permisos) {
    const client = await db_1.pool.connect();
    try {
        await client.query('BEGIN');
        await client.query(`UPDATE roles SET code = $1, name = $2, description = $3, updated_at = NOW() WHERE role_id = $4`, [code, name, description, roleId]);
 ...
 */
async function deleteRoleAndReassignUsersInDb(roleId) {
    const client = await db_1.pool.connect();
    try {
        await client.query('BEGIN');
