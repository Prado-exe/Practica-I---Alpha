    ...
    result.data.map(async (inst: any) => {
      if (inst.storage_key) {
        try {
          ...
          // 👇 TAMBIÉN DEVOLVEMOS LA URL ORIGINAL AQUÍ 👇
          return { ...inst, logo_url: presignedUrl }; 
...
export async function getInstitutionById(id: number) {
  ...
  // Lógica para firmar la imagen directamente en AWS S3
  if (inst.storage_key) {
    try {
      ...
      // Generamos la URL firmada oficial de Amazon S3
      const presignedUrl = await getSignedUrl(s3Client, command, { expiresIn: 3600 });
      
      // ✅ Eliminado el reemplazo de 'storage:9000' para usar la URL real de AWS
      return { ...inst, logo_url: presignedUrl };
    } catch (error) {
      console.error(`Error de AWS S3 firmando logo de institución ${id}:`, error);
      // En caso de error, devolvemos el objeto sin la URL firmada para no romper la respuesta
      return inst;
    }
  }
