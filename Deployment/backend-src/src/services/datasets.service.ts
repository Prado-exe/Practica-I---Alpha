...
export async function getDatasetById(id: number, requesterRole: string = 'guest', requesterInstitutionId: number | null = null) {
  // 1. Obtener datos (incluyendo el nuevo campo institution_access_level)
  const dataset = await fetchDatasetDetailsFromDb(id);
  if (!dataset) throw new AppError("Dataset no encontrado", 404);

  // 2. 🛡️ Validación de Privacidad (Blindaje Backend)
  if (dataset.institution_access_level === 'internal') {
    const isSuperAdmin = requesterRole === 'super_admin';
    const isMember = requesterInstitutionId && (requesterInstitutionId === dataset.institution_id);

    if (!isSuperAdmin && !isMember) {
      throw new AppError("Acceso Denegado: Este dataset es de carácter interno.", 403);
    }
  }

  // 3. Generación de URLs firmadas para AWS
    ...
    dataset.files = await Promise.all(
      dataset.files.map(async (file: any) => {
        ...
        try {
          ...
          console.error(`Error de AWS S3 en archivo ${file.storage_key}:`, error);
          return file;
        }
      })
    );
