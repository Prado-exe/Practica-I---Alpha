...
export async function getPublicNewsBySlug(slug: string) {
  ...
  // 🔄 Firmamos todos los archivos (portada y galería del detalle) para AWS S3
      ...
      news.files.map(async (file: any) => {
        if (file.storage_key) {
          try {
            ...
            // Generamos la URL firmada directamente desde la infraestructura de AWS
            const presignedUrl = await getSignedUrl(s3Client, command, { expiresIn: 3600 });
            
            // ✅ Asignamos la URL real de AWS sin reemplazos de localhost
            file.file_url = presignedUrl; 
          } catch (error) {
            console.error(`Error de AWS S3 firmando imagen de galería (${file.storage_key}):`, error);
          }
        }
        return file;
      })
...
export async function getPublicNews() { 
  ...
  // 🔄 Iteramos y firmamos las portadas para AWS S3
    ...
    newsList.map(async (n: any) => {
      if (n.cover_storage_key) {
        try {
          const command = new GetObjectCommand({ 
            Bucket: env.S3_BUCKET_NAME, 
            Key: n.cover_storage_key 
          });
          
          const presignedUrl = await getSignedUrl(s3Client, command, { expiresIn: 3600 });
          
          // ✅ Asignamos la URL oficial de AWS directamente
          n.cover_image = presignedUrl; 
        } catch (error) {
          console.error(`Error de AWS S3 firmando portada para noticia ${n.id}:`, error);
          n.cover_image = null;
        }
